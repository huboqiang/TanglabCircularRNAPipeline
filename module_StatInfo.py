...
class QcStat(object):
   ...
   def read_infile(self):
      ...
      self.ER_l_rate, self.ER_r_rate  = [ float(i) for i in f_1[4].split(';') ]
      self.Q20_l_rate,self.Q20_r_rate = [ float(i) for i in f_1[5].split(';') ]
      self.Q30_l_rate,self.Q30_r_rate = [ float(i) for i in f_1[6].split(';') ]
      self.GC_l_rate, self.GC_r_rate  = [ float(i) for i in f_1[7].split(';') ]
      self.N_remove  = l_line[2].split()[-1]
      self.Q_remove  = l_line[2].split()[-1]
      self.A_remove  = l_line[2].split()[-1]
